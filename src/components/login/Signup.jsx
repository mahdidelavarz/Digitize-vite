...
const SignUp = () => {
    ...
    const validationSchema = Yup.object({
        ...
        passwordConfirm: Yup.number().required('رمز عبور نمیتواند خالی باشد !')
    })
    ...
    return (
            <form className='w-full flex flex-col items-center text-sm gap-y-4'>
                <div className="w-full h-auto mt-4 flex flex-col  ">
                    ...
                    <input className=' outline-none  border px-4 py-2 mt-2 rounded-xl bg-white  dark:border-slate-500 dark:text-text-gray dark:bg-gray-700 text-black' type="text" name='name' placeholder='نام خودرا وارد کنید ...' {...formik.getFieldProps('name')} />
                ...
                <div className="w-full h-auto mt-4 flex flex-col ">
                    ...
                    <input className='outline-none border px-4 py-2 mt-2 rounded-xl bg-white  dark:border-slate-500 dark:text-text-gray dark:bg-gray-700' type="tel" name='password' placeholder='رمزعبور خود را وارد کنید ...' {...formik.getFieldProps('password')} />
                    {formik.errors.password && formik.touched.password && (<div className='text-red-400 mt-2 ml-4'>{formik.errors.password}</div>)}
                </div>
                <div className="w-full h-auto mt-4 flex flex-col ">
                    <label className='text-slate-700 dark:text-text-gray'>تایید رمزعبور :</label>
                    <input className='outline-none border px-4 py-2 mt-2 rounded-xl bg-white  dark:border-slate-500 dark:text-text-gray dark:bg-gray-700' type="tel" name='passwordConfirm' placeholder='رمزعبور خود را دوباره وارد کنید ...' {...formik.getFieldProps('passwordConfirm')} />
                    {formik.errors.password && formik.touched.password && (<div className='text-red-400 mt-2 ml-4'>{formik.errors.password}</div>)}
                </div>
                <button type='submit' className='w-full py-2 text-slate-50 bg-gradient-to-r from-blue-300 to-purple-600 mt-10 rounded-xl dark:from-gray-500' onClick={formik.handleSubmit}>ثبت نام</button>
